...
import { submitQuestion, verifyPasskeyDetailed } from '../services/questionsService';
...
export function SubmitView({
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    // 3. When the passkey input gets wrong or normal question:
    // Just let the message be sent directly, no indication that passkey was changed or wrong.
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter') {
      if (e.metaKey || e.ctrlKey) {
        ...
        const trimmed = content.trim();
        // If single line or matches master admin key, submit directly on Enter
        if (
